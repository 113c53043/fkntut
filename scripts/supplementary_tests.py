...
# 多目標設定列表
TARGET_ACCS = [0.90, 0.98, 0.985, 0.995]
...
def check_bob_accuracy(model, sampler, z_current, prompt, secret_key, gt_bits):
    try:
        ...
        with torch.no_grad(), autocast("cuda"):
            ...
            # [關鍵] 轉成 Tensor 供攻擊函式使用 (模擬真實儲存)
            # x_samples is [1, 3, 512, 512], range [0, 1]
            # robust_eval 預期 range [-1, 1]
            img_tensor = x_samples * 2.0 - 1.0

        # 2. [修正] 提升攻擊強度至 JPEG(50)
        # 這樣才能強迫模型進行優化，避免 Step 1 就達標
        temp_name = os.path.join(OUTPUT_DIR, "temp_calib")
        jpeg(img_tensor, 50, tmp_image_name=temp_name)
        
        # 讀回攻擊後的圖片
        attacked_img_path = temp_name + ".jpg"
        if os.path.exists(attacked_img_path):
             attacked_tensor = load_512(attacked_img_path).cuda()
             os.remove(attacked_img_path)
        else:
             return 0.0

        # 3. Image -> Inversion (Bob)
        c = model.get_learned_conditioning([prompt])
        uc = model.get_learned_conditioning([LONG_NEGATIVE_PROMPT])
        
        with torch.no_grad(), autocast("cuda"):
            init_latent = model.get_first_stage_encoding(model.encode_first_stage(attacked_tensor))
...
def run_comparative_calibration(model, sampler):
    ...
    print(f"   Condition: Under JPEG(50) Attack (Harder constraint)")
    ...
    base_prompts = [
        ...
        "Cyberpunk street", "Minimalist logo", "Starry night", "Crowded market"
    ]
    ...
    for i in pbar:
        ...
        for step in range(MAX_ITERS):
            ...
            # 2. Check Accuracy (with Attack)
            # 因為要觀察趨勢，我們每步都檢查，不再只檢查未達標的
            # 但為了性能，如果全部達標就 break
...
def analyze_and_plot(data):
    print("\n" + "="*80)
    print("COMPARATIVE ANALYSIS REPORT (Under JPEG-50 Attack)")
    print("-" * 80)
