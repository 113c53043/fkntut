...
from srnet_model import SRNet

CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
ROOT_DIR = os.path.dirname(CURRENT_DIR)
DATA_DIR = os.path.join(ROOT_DIR, "training_data")
WEIGHTS_DIR = os.path.join(ROOT_DIR, "weights")
SAVE_PATH = os.path.join(WEIGHTS_DIR, "srnet_best.pth")
...
def train():
    ...
    print(f"🚀 [SRNet] 訓練開始 (Augmentation Enabled)...")
    ...
    # SRNet 參數多，batch_size 設小一點避免 OOM
    train_loader = DataLoader(train_ds, batch_size=16, shuffle=True, num_workers=4)
    val_loader = DataLoader(val_ds, batch_size=16, shuffle=False, num_workers=4)
    
    model = SRNet().to(device)
    criterion = nn.CrossEntropyLoss()
    # SRNet 使用 Adamax 效果較好
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_acc:
            ...
            print(f"  💾 保存最佳 SRNet (Acc: {val_acc:.2f}%)")
