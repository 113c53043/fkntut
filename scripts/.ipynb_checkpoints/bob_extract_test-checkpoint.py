...
def main():
    parser = argparse.ArgumentParser(description="Bob Test Version")
    parser.add_argument("--img_path", type=str, required=True)
    parser.add_argument("--prompt", type=str, required=True)
    parser.add_argument("--secret_key", type=int, required=True)
    parser.add_argument("--gt_path", type=str, help="Ground Truth Path")
    ...
    print(f"[Bob] 使用金鑰提取: {opt.secret_key}")
    ...
    with torch.no_grad(), autocast("cuda"):
        ...
        z_T_hat, _ = sampler.sample(
            ...
            x_T=init_latent,
            DPMencode=True, DPMdecode=False
        )
    ...
    try:
        ...
        if cipher_len <= 0 or cipher_len > len(final_payload_bytes) - 2:
            raise ValueError(f"Invalid Length: {cipher_len}")
            
        real_ciphertext = final_payload_bytes[2 : 2+cipher_len]
        
        # 檢查對齊
        if len(real_ciphertext) % 16 != 0:
            raise ValueError(f"Not Aligned: {len(real_ciphertext)}")
        ...
        if opt.gt_path and os.path.exists(opt.gt_path):
            ...
            gt_hash = hashlib.sha256(gt_data).hexdigest()
            restored_hash = hashlib.sha256(real_data).hexdigest()
            
            if gt_hash == restored_hash:
                print("="*30)
                print("🎉 雙層驗證成功！(ECC + AES)")
                print("📊 Zero-Error Verification: PASSED")
                print("="*30)
            else:
